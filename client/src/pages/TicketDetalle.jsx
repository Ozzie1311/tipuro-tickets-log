...
const TicketDetalle = () => {
  ...
  const handleResolver = async () => {
    try {
      ...
      setError('Error al resolver el ticket', error)
    }
  }
      ...
      <div className='px-4 flex flex-col gap-3 pb-10'>
        ...
        <div className='bg-zinc-900 rounded-2xl p-5 flex flex-col gap-3'>
          <div className='flex items-start justify-between gap-3'>
            ...
            <span
              ...
              style={{
                backgroundColor: ticket.estado_color + '33',
          ...
          <div className='border-t border-zinc-800 pt-3 flex flex-col gap-1.5'>
            ...
            <div className='flex'>
              ...
              <span className='ml-1 text-zinc-300 text-xs font-medium'>
                {ticket.resuelto_por || 'Sin resolver'}
              </span>
            </div>
            {ticket.actualizado_por && (
              <div className='flex '>
        ...
        {!ticket.resuelto && (
          <button
            ...
            className='bg-green-500/20 text-green-400 rounded-2xl p-4 font-semibold text-md active:scale-95 transition-transform duration-150'
          >
            Resolver ticket ✔
          </button>
        )}
          ...
          <div className='fixed inset-0 bg-black/70 backdrop-blur-sm flex items-end justify-center z-50 animate-fadeIn'>
            <div className='bg-zinc-900 w-full rounded-t-3xl p-6 flex flex-col gap-4 pb-10 animate-slideUp'>
              ...
              <p className='text-zinc-400 text-sm text-center'>
                Una vez resuelto el ticket no se podra editar, estas seguro?
              </p>
              <button
                onClick={handleResolver}
                className='bg-green-500/10 text-green-400 rounded-xl p-3 font-semibold active:scale-95 transition-transform duration-150'
              >
                Sí, resolver ✔
              </button>
              <button
                onClick={() => setMostrarConfirmacion(false)}
                className='bg-red-500/10 text-red-400 rounded-xl p-3 font-semibold active:scale-95 transition-transform duration-150'
              >
                Cancelar
              </button>
            </div>
        ...
        <form onSubmit={handleComentario} className='flex flex-col gap-2'>
          ...
          <button
            type='submit'
            className='bg-blue-500/20 text-blue-400 rounded-xl p-3 font-semibold active:scale-95 transition:transform duration-150'
          >
            Agregar comentario
          </button>
        </form>
